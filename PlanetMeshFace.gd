...
func regenerate_mesh(planet_data: PlanetData):
	...
	var num_indices: int = (resolution - 1) ** 2 * 6
	...
	for y in range(resolution):
		for x in range(resolution):
			...
			var biome_index = planet_data.biome_percent_from_point(point_on_unit_sphere)
...
func _update_mesh(arrays: Array, planet_data: PlanetData):
	...
	material_override.set_shader_parameter("min_height", planet_data.min_height - 0.01)
...
func _process(delta):
	if !Engine.is_editor_hint() && Globals.ready_to_coll:
